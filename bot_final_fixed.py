...
def cleanup_old_keys():
    """Delete revoked keys after 24 hours and expired keys after 12 hours"""
    try:
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        ...
        # Delete revoked keys older than 24 hours
        revoked_cutoff = now - timedelta(hours=24)
        cursor.execute("""
            DELETE FROM keys WHERE status = 'revoked' AND created_at < ?
        """, (revoked_cutoff.isoformat(),))
        
        # Delete expired keys older than 12 hours past expiration
        expired_cutoff = now - timedelta(hours=12)
        cursor.execute("""
            DELETE FROM keys WHERE status = 'used' AND expires_at < ?
        """, (expired_cutoff.isoformat(),))
        
        conn.commit()
        conn.close()
        logger.info("Cleanup completed: old keys deleted")
    ...
    try:
        ...
        if keys:
            for key_code, duration, status, name, username, expires_at in keys:
                ...
                user_info = f"@{username}" if username and username != "N/A" else "(Not redeemed)"
                status_text = "ACTIVE" if status == "active" else status.upper()
                response += f"{status_emoji} `{key_code}` ({duration}) - {status_text}\n   User: {user_info} ({name})\n\n"
        else:
...
async def call_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Send notification to the caller (user or admin)
        call_msg = f"""
📞 **Call Initiated**
...
"""
        try:
            # Send to the user who made the call
            await context.bot.send_message(chat_id=user_id, text=call_msg, parse_mode="Markdown")
        except:
            pass
